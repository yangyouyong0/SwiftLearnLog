...
class PhotoViewerViewController: UIViewController, UIScrollViewDelegate, UIPopoverPresentationControllerDelegate, UIActionSheetDelegate {
  ...
  override func viewDidLoad() {
    ...
    func loadPhoto(){
        Alamofire.manager.request(Five100px.Router.PhotoInfo(self.photoID, .Large)).validate().responseObject { (_, _, photoInfo:PhotoInfo?, error) -> Void in
            if error == nil {
                ...
                Alamofire.manager.request(Method.GET, photoInfo!.url).validate().responseImage({ (_, _, image, error) -> Void in
  ...
  func downloadPhoto() {
    Alamofire.manager.request(Five100px.Router.PhotoInfo(photoInfo!.id, .XLarge)).validate().responseObject(){
...
//            Alamofire.manager.download(.GET, imageURL, destination: destination)
            ...
            Alamofire.manager.download(.GET, imageURL, destination: destination).progress {
